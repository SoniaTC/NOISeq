...
The expression data may be the read counts but also normalized expression data such as RPKM values or any other normalized expression values.


\subsection{Factors}
Factors are given in a data.frame object like follows. For Marioni's data, we obviously have the factor ``Tissue'', but we may also define another toy factor (``TissueRun'') 
...
(we have 2 sequencing runs) with the tissue and then has four levels: ``Kidney\_1'', ``Liver\_1'', ``Kidney\_2'' and ``Liver\_2''. 
...
Please note, that these objects may not contain the same number of features or in the same order than the expression data. 
...
The length, GC content or biological groups (e.g. biotypes), may be vectors, matrices or data.frames. If they are vectors, the names of the vector must be the feature names or ids. 
In they are matrices or data.frame objects, the feature names or ids must be in the row names of the object. 
...
To read these data and start working with all the functionalities from \noiseq{} package, the function \code{readData} has to be used. An example on how it works is shown below. 
...
biological samples to the expression quantification, there are many steps in which errors may be introduced, despite of the many procedures 
...
can be chosen. For instance, these plots will be useful to see which kind of features (e.g. genes) are being detected in our RNA-seq samples, how many counts they are 
being detected with, which technical biases are present, etc. 



\subsection{Generating data for exploratory plots}
There are six types of exploratory plots that can be obtained and that will be described in detail in the following sections. To generate any of these plots, 
first of all, \code{dat} function must be applied on the input data to obtain the information to be plotted. The user must specify for which type of plot the data are to be 
computed (argument \code{type}). Once the data for the plot have been generated with \code{dat} function, the plot will be drawn with the \emph{explo.plot} function. For instance:
...
\begin{itemize}
 ...
 \item Sequencing depth \& Expression Quantification: Would it be better to increase of sequencing reads to detect more features? Are there too many features with low counts?
...
If a biological classification of the features is provided (e.g. Ensembl biotypes), the following plots may be obtained to see which kind of features are being detected.
...
This is an example on how to use the corresponding functions, first to generate the data to be plotted and then to draw the plot. Since factor=NULL, 
the data for the plot are computed separately for each sample. If factor is a character vector indicating the experimental condition for each sample,
the samples are aggregated within each condition by summing up the counts and the data for the plot are computed for each condition. In this case, 
...
The plots in this section can be generated by only providing the expression data, since no other biological information is required. Their purpose is to assess if the sequencing
...
detected features would change when increasing sequencing depth. This plot may be generated considering all the features or for each biological group (biotypes), if available.
...
It is possible to compare more than 2 samples, but then the information in the right axis is not provided because it would be difficult to distinguish so many bars. You can compare
...
Fig. \ref{fig_boxplot2} shows this information for the biotype "protein\_coding" and can be generated with the following code on the "countsbio" object obtained in the previous
section.
...
Features with low counts are, in general, less reliable and may introduce noise in the data that makes it more difficult to extract the relevant information, for instance, the
...
the bars correspond to the total number of features and above each bar, the sequencing depth of each sample (in million reads) is given. The following code is for drawing this
figure.
...
Prior to perform further analyses such as differential expression, it is essential to normalize data, i.e. transform the data in such a way that the samples become
comparable and that the effect of technical biases is removed from the expression estimation. The plots presented in this section are very useful to detect the possible biases in
...
shown in the plot as well as the fitted regression curve. If the model p-value is significant and R2 value is high, the expression depends on the feature length and the curve show
the type of dependence. 
 
An example of this plot is in Fig. \ref{fig_length}. In this case, the "lengthbias" data were generated for each condition (kidney and liver) using the argument \code{factor}.
...
More details about the fitted spline regression models can be obtained by using the \code{show} function as below:
...
data. The GC content is divided in intervals (bins) containing 200 features and the middle point of each bin is
depicted in X axis. For each bin, the 5\% trimmed mean of the corresponding expression values is
computed and depicted in Y axis. If the number of samples or conditions to appear in the plot is 2 or less and no biotype is specified (toplot = "global"), a diagnostic test is
...
are
shown in the plot as well as the fitted regression curve. If the model p-value is significant and R2 value is high, the expression depends on the feature GC content and the curve
show the type of dependence. 
...
When two samples have different RNA composition, the distribution of sequencing reads across the features is also different in such a way that although a feature had the same
number of read counts in both samples, it would not mean that it was equally expressed in both. To check if the distribution of counts is similar for all samples, the "cd" plot
and diagnostic test may be used. A Kolmogorov-Smirnov test is conducted for each pair of samples and the minimum FDR-adjusted p-value is displayed on screen or it can be
...
In the plot, it can be seen that the distributions are different for kidney and liver samples. For example, in kidney 20\% of the genes account for about 65\% of the reads, while
...
that is optimized for the use of biological replicates. All the three methods take read counts from RNA-seq as the expression values, but also previously normalized data as well
as read counts from other NGS technologies.
...
Normalization functions \code{rpkm}, \code{tmm} and \code{uqua} can be used by themselves. Please, find below some examples on how to do that:
...
If the length of the features is provided and any of the three normalization methods is chosen, the expression values are divided by $length^{lc}$ ($lc = 1$ for RPKM method). Thus,
...
\item \textbf{Wilcoxon test} (method 2): For each feature and condition,  $H_0: m=0$ versus $H_1: m>0$ is
tested, where $m$ is the median of counts per condition. Features with
...
from a multinomial distribution, so be careful with your results when having no replicates, since they are not showing real differential expression, but they are only an
approximation to show you which genes are presenting a higher change between conditions.
...
In RNA-seq or similar sequencing technologies, counts from technical replicates (e.g. lanes) can be summed up. Thus, this is the way the algorithm summarizes the information from
...
In order to afterwards compute the null density $f_{0}$, we first need to estimate the values of the $\theta$-scores for features with no change
...
expression values across replicates using the k-means method. For each cluster of genes $k$, we considered the expression values of all the
...
"theta" for \code{NOISeqBIO}); probability of differential expression ("prob"); "ranking", which is a summary statistic of "M" and "D" values equal to $-sign(M) \times \sqrt{M^2 +
D^2}$, than can be used for instance in gene set enrichment analysis (only for \code{NOISeq}); "Length" of each feature (if provided); "GC" content of each feature (if provided);
...
\item
  \texttt{pnr}: Percentage of the total sequencing depth to be used in each simulated replicate (only when there are not replicates available). If, for instance, pnr = 0.2 , each
...
choose if plotting all the chromosomes or only some of them, and also if the chromosomes are depicted consecutively (useful for prokaryote organisms) or separately (one per line).
