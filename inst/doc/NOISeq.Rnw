...
<<factors>>=
myfactors = data.frame(Tissue=c("Kidney","Liver","Kidney","Liver","Liver",
                                "Kidney","Liver","Kidney","Liver","Kidney"),
                       TissueRun = c("Kidney_1","Liver_1","Kidney_1","Liver_1",
                                     "Liver_1","Kidney_1","Liver_1","Kidney_2",
                                     "Liver_2","Kidney_2"))
@ 
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.9\textwidth]{img/biodetection.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.5\textwidth]{img/countdistr.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{img/counts_boxplot_1.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.45\textwidth]{img/counts_boxplot_2.pdf}
...
The plot in Fig. \ref{fig_length} is to describe the median length of detected features within each sample and per biotype when increasing sequencing depth. In this example, we plotted 4 samples (from 3 to 6) and the "protein\_coding" biotype. First we compute the values for all the samples and biotypes and then we plot the chosen samples and/or biotypes.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.6\textwidth]{img/length.pdf}
...
First, we have to generate the saturation data with the function \code{dat} and then we can use the resulting data to obtain, for instance, the plots in Fig. \ref{fig_sat1} and \ref{fig_sat2} by using \emph{explo.plot}. When \code{newdetections} parameter is set to TRUE, a right axis and bars are drawn to show the number of new features detected when increasing the sequencing depth in one million of reads. If more than 2 samples are to be plotted, this parameter is automatically set to FALSE because it is difficult to visualize the newdetection bars with so many samples.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.5\textwidth]{img/saturation1.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.5\textwidth]{img/saturation2.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.6\textwidth]{img/summary_expr.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.6\textwidth]{img/summary_MD.pdf}
...
The following code shows how to draw the Manhattan plot from the output object returned by \code{NOISeq}. In this case, using Marioni's data, the log-expression is represented for three chromosomes (see Fig. \ref{fig_manhattan}). Gene expression is represented in gray. Lines above 0 correspond to the first condition under comparison (kidney) and lines below 0 are for the second condition (liver). Genes up-regulated in the first condition are highlighted in red, while genes up-regulated in the second condition are highlighted in green. The blue lines on the horizontal axis (Y=0) correspond to the annotated genes. X scale shows the location in the chromosome.

<<>>=
DE.plot(mynoiseq, chromosomes = c("I","II"), log.scale = TRUE,
...
\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{img/manhattan.pdf}
\caption{Manhattan plot for chromosomes I, and II}
\label{fig_manhattan}
\end{figure}

When all (or many) of the chromosomes are to be plotted, it is advisable to save the figure in a file, for instance, a pdf file (as in the following code), in order to get a better visualization with the zoom.
